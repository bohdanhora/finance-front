...
export function ChartPieByCategory() {
    ...
    const generateColor = (index: number): string => `var(--chart-${(index % 10) + 1}, hsl(${index * 36}, 70%, 60%))`;
    ...
    return (
        <ContentWrapper className={twMerge("flex flex-col items-center gap-4", showChart ? "w-full" : "w-fit")}>
            <div className="text-center">
                <h2 className="text-xl font-semibold">{tChart("expensesByCategory")}</h2>
                <p className="text-muted-foreground text-sm">{tChart("excludingIncomeTransactions")}</p>
            </div>
            ...
            {showChart && (
                <>
                    <div className="flex flex-col md:flex-row w-full max-w-xl items-center gap-8">
                        <ChartContainer config={chartConfig} className="w-full max-w-xl aspect-square">
                            <PieChart>
                                <ChartTooltip
                                    cursor={false}
                                    content={
                    ...
                    <div className="text-sm text-muted-foreground text-center">
                        <div className="flex items-center justify-center gap-2 font-medium">
                            {tChart("basedOnRecentExpenses")} <TrendingUp className="h-4 w-4" />
                        </div>
                        <div>
